...
<section id="content-wrap" class="col">
    <div class="row">
        <div class="col">
    ...
    <div class="comments-wrap">
        <div id="comments" class="row">
            @if(session('warning'))
                ...
                @endcomponent
            @endif
            <h3>{{ $post->valid_comments_count }}
                {{ trans_choice(__('comment|comments'), $post->valid_comments_count) }}</h3>
                    ...
                    <form method="post" action="{{ route('posts.comments.store', [$post->id]) }}">
                        ...
                        <div class="message form-field">
                            @if($errors->has('message'))
                                ...
                                @endcomponent
                            @endif
                            <textarea name="message" id="message" class="full-width" placeholder="@lang('Your message')"
                ...
                $.ajax({
                    ...
                    .fail(function (data) {
                        ...
                        $.each(errors, function (index, value) {
                            value = value[0].replace(index, '@lang('
                                message ')')
                            $('form.comment-form textarea[name="' + index + '"]').prev().text(
                                value)
                        });
                    });
            }
                ...
                $.ajax({
                    ...
                    .fail(function (data) {
                        ...
                        $.each(errors, function (index, value) {
                            value = value[0].replace(index, '@lang('
                                message ')')
                            $('form.reply-form textarea[name="' + index + '"]').prev().text(
                                value)
                        });
                    });
            }
